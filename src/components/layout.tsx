  ...
  if (!user) {
    return (
      <div className="flex items-center gap-2.5">
        ...
        <Link to="/rezervo" className="h-9 px-4 inline-flex items-center rounded-[10px] bg-primary-600 text-primary-50 text-[13.5px] font-semibold hover:bg-primary-700 transition-colors shadow-xs">{t("nav.book")}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} aria-label="Menuja e llogarisë" className="flex items-center gap-2 rounded-full p-0.5 pr-1 hover:bg-ink/[0.04] transition-colors">
        ...
        <IChevD size={12} className="text-mute" />
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
          <div className="absolute right-0 mt-2 bg-card border border-line rounded-xl shadow-pop py-1.5 w-56 z-50 anim-scale-in">
...
export function PublicHeader() {
  ...
  return (
    <header className={cls("sticky top-0 z-40 transition-all duration-200 border-b",
      scrolled ? "bg-card/92 backdrop-blur-md border-line shadow-xs" : "bg-transparent border-transparent")}>
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {portal ? (
            <div className="flex items-center gap-2.5">
              <Link to={portal} className="h-9 px-4 inline-flex items-center rounded-[10px] bg-ink text-paper text-[13.5px] font-semibold hover:bg-primary-900 transition-colors">{t("nav.portal")}</Link>
              <UserMenu />
            </div>
          ) : (
            <>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-line bg-card px-4 pt-3 pb-5 anim-fade-in">
...
export function PublicFooter() {
  ...
  const lnk = "block text-[13.5px] text-paper/65 hover:text-paper py-1.5 transition-colors";
  return (
    <footer className="bg-ink text-paper/80 mt-24">
      <div className="max-w-6xl mx-auto px-4 py-14 grid gap-10 md:grid-cols-[1.5fr_1fr_1fr_1fr]">
        <div>
          <Logo dark />
        ...
        <>
          ...
          <div className="absolute right-0 mt-2 bg-card border border-line rounded-xl shadow-pop w-[min(92vw,360px)] z-50 anim-scale-in overflow-hidden">
            ...
            <div className="max-h-[380px] overflow-y-auto">
              ...
              {recent.map((n) => (
                <div key={n.id} className="px-4 py-3 border-b border-line/70 last:border-0">
...
export function PortalShell({ nav, title, children }: { nav: NavItem[]; title: string; children: React.ReactNode }) {
  ...
  const sidebar = (
    <div className="flex flex-col h-full">
      <div className="px-4 h-14 flex items-center border-b border-line/70 shrink-0">
        <Logo />
      </div>
      <div className="px-5 pt-4 pb-2">
        <p className="text-[9.5px] font-mono uppercase tracking-[0.22em] text-mute/80">{title}</p>
      </div>
      <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto pb-4 no-scrollbar" aria-label={title}>
        {nav.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.end}
            className={({ isActive }) => cls(
              "relative flex items-center gap-2.5 px-3 h-9 rounded-[9px] text-[13px] font-semibold transition-all duration-150",
              isActive ? "bg-primary-600/[0.09] text-primary-800" : "text-ink-2 hover:bg-ink/[0.04] hover:text-ink",
            )}>
            {({ isActive }) => (
              <>
                {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-4 rounded-full bg-primary-600" aria-hidden="true" />}
                <span className={cls("shrink-0", isActive ? "text-primary-700" : "text-mute")}>{item.icon}</span>
                {item.label}
              </>
            )}
          </NavLink>
      ...
      <div className="p-3 border-t border-line/70">
        <div className="flex items-center gap-2.5 px-1.5 py-1.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[12.5px] font-bold text-ink truncate leading-tight">{user.full_name}</p>
            <p className="text-[10.5px] text-mute truncate">{ROLE_LABEL[user.role] ?? user.role}</p>
          </div>
          <button onClick={() => { logout(); nav2("/"); }} className="w-8 h-8 rounded-lg hover:bg-bad-soft text-mute hover:text-bad flex items-center justify-center transition-colors" title={t("portal.logout")} aria-label={t("portal.logout")}>
            <ILogout size={15} />
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex bg-paper">
      <aside className="hidden lg:block w-[228px] shrink-0 bg-card border-r border-line sticky top-0 h-screen">
        {sidebar}
      </aside>
      {mobileNav && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div className="absolute inset-0 bg-ink/45 anim-fade-in" onClick={() => setMobileNav(false)} />
          <aside className="absolute left-0 top-0 h-full w-[260px] bg-card border-r border-line anim-fade-in shadow-pop">{sidebar}</aside>
        </div>
      )}
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <main className="flex-1 p-4 lg:p-7 max-w-[1220px] w-full mx-auto">{children}</main>
      </div>
    </div>
  );
}
